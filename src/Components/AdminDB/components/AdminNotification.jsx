...
export default function AdminNotification() {
  ...
  useEffect(() => {
      const getadminnotif = () => {
          ...
          const adminnotifArray = [];
          
          //console.log(props)

          getDocs(collection(db, "users","admin123@gmail.com","notification")).then((querySnapshot) => {
              querySnapshot.forEach((doc) => {
                  // console.log(doc.id, " => ", doc.data());
                  adminnotifArray.push({ ...doc.data(), id: doc.id })
              });
              setadminnotif(adminnotifArray)
              // console.log('done')
